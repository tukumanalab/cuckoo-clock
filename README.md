...
3Dプリンターで製作できるオルゴール（ミュージックボックス）用カムの STL ファイルを、ブラウザ上で生成する Web アプリです。

**[大人の科学マガジン　あたらしい鳩時計](https://hon.gakken.jp/book/1575098600)（学研）の付録に付いているオルゴール機構で使えるカムのジェネレーターです。** このキットの機構にそのままセットできる寸法でカムを出力します。

![アプリのスクリーンショット](docs/screenshot.png)

## 概要

オルゴール機構の「琴爪（きんそう）」は、カムの歯（突起）の高さに応じてどの音が鳴るかが決まります。このアプリでは 25 分割されたカムの各セクターに音符を割り当て、好みのメロディを奏でるカムを設計・出力できます。
...
4. 「STL をダウンロード」で 3D プリント用ファイル（`melody_cam.stl`）を取得
...
| STL をダウンロード | 現在のメロディをカム形状（STL）として書き出し |
...
ピアノロールで「ラ」と打ち込むと、ブラウザは **A4＝440Hz** を鳴らしますが、実機のカムが鳴らすのは **A4 ではありません**。試聴と実機の音は、次の 2 つの点で違います。
...
カム上に「休符」という概念はありません。エディター上で何も選択していないビート（休符）は、STL 出力時に**直前のビートと同じ外径**として扱われます。これによりすべてのセクターが機構に確実にかかり、同じ音が連続して鳴る形になります。

### 実測値（参考）

5 つの代表的な外径について、単音テストカムを実機にかけてマイクで録音・解析した結果です。この 5 点は機構が出せる音の **代表サンプル** であり、間の径（例：φ36.67mm）でも音は鳴ります（連続的に変化します）。

| 外径 | 実測音名（ドレミ／音名） | 代表周波数 | 備考 |
|---|---|---|---|
| φ40mm | **シ♭7（ラ#7） / Bb7（A#7）** | ~3709 Hz | ボディ共鳴との差分で同定 |
| φ37.5mm | ファ#7 / F#7（参考値） | ~2980 Hz | 5音サイクルカムからの推定 |
